...
class HomeScreen extends GetView<HomeController> {
  ...
  Widget build(BuildContext context) {
    ...
    final recommendationProductController = Get.find<RecommendationProductController>();
    ...
    return Scaffold(
      ...
      appBar: PreferredSize(
        ...
        child: controller.obx((state) {
          return UEAppBar(transparent: state?.showWhiteAppBar == false, title: null);
        })
      ),
      body: RefreshIndicator(
        ...
        child: recommendationProductController.obx(
          (recommendations) => main(context, recommendations, scrollController, recommendationProductController),
          onLoading: main(context, null, scrollController, recommendationProductController)
        ),
      )
    );
  }
  ...
  Widget main(
    ...
    if (homeState == null) return const SafeArea(child: Center(child: CircularProgressIndicator()));
    final recommendationNoOfColumn = sizer.Device.screenType == sizer.ScreenType.tablet ? (sizer.Device.safeWidth / 200).ceil() : 2;
    return MediaQuery.removePadding(
      ...
      child: ListView.builder(
        ...
        itemCount: HomeSection.values.length + ((recommendations?.products.length ?? 0) ~/ recommendationNoOfColumn),
        itemBuilder: (context, index) {
          if (HomeSection.values.length > index) {
            ...
            switch (section) {
              ...
              default: return Text("Not Implemented $section");
            }
          }
          final recommendationProducts = recommendations?.products ?? [];
          int currentRow = (index - HomeSection.values.length) * recommendationNoOfColumn;
          ...
          for (var col = 0; col < recommendationNoOfColumn; col++) {
            if (recommendationProducts.length > currentRow + col) {
              ...
              row.children.add(Expanded(child: NewProducttile(product: product)));
            } else {
          ...
          return Padding(padding: const EdgeInsets.symmetric(vertical: 3.5), child: row);
        },
      ));
  });
}
...
enum HomeSection {
  ...
  MainBanner, MainMenu, Banner, FlashSale, OfficialBrands, SuperDeal, PromoPembayaran, BestSeller, Recommendation
}